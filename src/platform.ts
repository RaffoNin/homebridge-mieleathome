...
export class MieleAtHomePlatform implements DynamicPlatformPlugin {
  ...
  public readonly reconnectEventServerInterval: number = parseInt(
    <string>this.config.reconnectEventServerInterval ||
      `${DEFAULT_RECONNECT_EVENT_SERVER_INTERVAL_MIN}`
  );

  public readonly language = this.config.language || "";
  public readonly disableStopActionFor: string[] =
    <string[]>this.config.disableStopActionFor || [];
  public readonly disableTempSensorFor: string[] =
    <string[]>this.config.disableTempSensorFor || [];
  public readonly disableSetTargetTempFor: string[] =
    <string[]>this.config.disableSetTargetTempFor || [];
  ...
  private async discoverDevices() {
    try {
      ...
      for (const deviceId of allDeviceIds) {
        ...
        const deviceObject = {
          ...
          firmwareRevision:
            ...
            "Unknonw Miele firmware",
          displayName:
            device.ident.deviceName ||
            device.ident.type.value_localized ||
            MieleDeviceIds[device.ident.type.value_raw] ||
            `Unnamed ${deviceId}`,
          modelNumber:
            device.ident.deviceIdentLabel.techType || "Unknown Miele model",
        };

        this.log.info(
          `Discovered device: id: ${deviceObject.uniqueId}, ` +
        ...
        if (existingAccessory) {
          ...
          this.log.info(
            "Re-creating existing accessory with data from cache:",
            existingAccessory.displayName
          );
          existingAccessory.context.device = deviceObject; // Update cache with newest data.

          const accessoryObj = this.constructPlatformAccessory(
          ...
          if (!accessoryObj) {
            this.log.error(
              "Retrieved accessory from cache, but its raw type value is not a supported device. " +
      ...
      if (response.config && response.response) {
        this.log.error(
          `Miele API request ${response.config.url} failed with status ${response.response.status}: ` +
  ...
  //-----------------------------------------------------------------------------------------------
  // Construct accessory
  private constructPlatformAccessory(
    raw_id: number,
    ...
    switch (raw_id) {
      ...
      case MieleDeviceIds.Dryer:
        return new MieleWasherDryerPlatformAccessory(
          ...
          this.disableStopActionFor.includes(MieleDeviceIds[raw_id]),
          true
        ); // Dryer is estimated to not have a target temp attribute.

      case MieleDeviceIds.Fridge:
      case MieleDeviceIds.Freezer:
      case MieleDeviceIds.FridgeFreezer:
        return new MieleFridgePlatformAccessory(
          this,
          accessory,
          this.disableStopActionFor.includes(MieleDeviceIds[raw_id]),
          this.disableSetTargetTempFor.includes(MieleDeviceIds[raw_id])
  ...
  private verifyConfig(configParameterStr: string) {
    if (!this.config[configParameterStr]) {
      this.log.error(
        `Configuration parameter "${configParameterStr}" invalid. Will attempt to continue, ` +
