...
import {
  MieleStatusResponse,
  MieleState,
  MieleStatusResponseTemp,
} from "./mieleBasePlatformAccessory";
...
abstract class MieleBaseCharacteristic implements IMieleCharacteristic {
  ...
  constructor(
    ...
    this.deviceName = this.service.getCharacteristic(
  ...
  get(callback: CharacteristicGetCallback) {
    this.platform.log.debug(
      `${this.deviceName}: Returning ${this.value} for ${this.characteristic.name}.`
    );
  ...
  protected undoSetState(value: CharacteristicValue) {
    if (value !== this.value) {
      this.platform.log.info(
        `${this.deviceName}: Reverting state to ${this.value}.`
      );
...
abstract class MieleBinaryStateCharacteristic extends MieleBaseCharacteristic {
  ...
  update(response: MieleStatusResponse): void {
    this.platform.log.debug(
      `${this.deviceName}: Update received for ${this.characteristic.name} raw value: ${response.status.value_raw}.`
    );
    ...
    if (this.inactiveStates) {
      ...
      throw new Error(
        `${this.deviceName}: Neither active or inactive states supplied. Cannot determine state.`
      );
    }
...
export class MieleOutletInUseCharacteristic extends MieleBinaryStateCharacteristic {
  constructor(
    ...
    super(
      ...
      inactiveStates,
      activeStates,
      platform.Characteristic.OutletInUse,
      0,
      1
    );
  }
...
export class MieleWritableBinaryStateCharacteristic extends MieleBinaryStateCharacteristic {
  ...
  async set(value: CharacteristicValue, callback: CharacteristicSetCallback) {
    this.platform.log.debug(
      `${this.deviceName}: Set characteristic ${this.characteristic.name} to: ${value}`
    );
    ...
    if (this.disableDeactivateAction && value === this.offState) {
      this.platform.log.info(
        `${this.deviceName} (${this.serialNumber}): Ignoring deactivate request. ` +
    ...
    try {
      ...
      this.platform.log.debug(
        `${this.deviceName} (${this.serialNumber}): Allowed process actions: ${response.data.processAction}.`
      );
      ...
      if (response.data.powerOn === true && value === this.onState) {
        ...
        const response = await axios.put(
          this.platform.getActionsUrl(this.serialNumber),
          { processAction: mieleProcesAction },
          this.platform.getHttpRequestConfig()
        );
        this.platform.log.debug(
          `${this.deviceName}: Process action response code: ${response.status}: "${response.statusText}"`
        ...
        this.platform.log.info(
          `${this.deviceName} (${this.serialNumber}): Ignoring request to set device to HomeKit value ${value}. ` +
            `Miele action "${MieleProcessAction[mieleProcesAction]}" (${mieleProcesAction}) not allowed in current device state. ` +
            `Allowed Miele process actions: ${
              response.data.processAction
                ? "<none>"
                : response.data.processAction
            }`
        );
...
export class MieleActiveCharacteristic extends MieleWritableBinaryStateCharacteristic {
  constructor(
    ...
    super(
      ...
      MieleProcessAction.Stop,
      platform.Characteristic.Active.INACTIVE,
      MieleProcessAction.Start,
...
export class MieleRemainingDurationCharacteristic extends MieleBaseCharacteristic {
  ...
  update(response: MieleStatusResponse): void {
    let value =
      response.remainingTime[0] * 3600 + response.remainingTime[1] * 60;
    this.platform.log.debug(
      `${this.deviceName}: Remaining Duration update received: ${value}[s]`
    );
    ...
    if (typeof value !== "number" || Number.isNaN(value)) {
      this.platform.log.debug(
        `${this.deviceName}: Prevented setting NaN or another non-number type for Remaining Duration.`
      );
      value = 0;
    }
...
export class MieleTempCharacteristic extends MieleBaseCharacteristic {
  ...
  update(response: MieleStatusResponse): void {
    ...
    if (tempArray.length > 0) {
      ...
      this.platform.log.debug(
        `${this.deviceName}: Zone ${this.DEFAULT_ZONE} ${
          TemperatureType[this.type]
        } ` +
      ...
      // Clip to min and max value.
      const characteristicObj = this.service.getCharacteristic(
        this.characteristic
      );
...
export class MieleTargetTempCharacteristic extends MieleTempCharacteristic {
  ...
  async set(value: CharacteristicValue, callback: CharacteristicSetCallback) {
    ...
    if (this.disableSetTargetTemp) {
      this.platform.log.info(
        `${this.deviceName} (${this.serialNumber}): Ignoring set ${
          TemperatureType[this.type]
        } temperature request. ` +
          `User disabled modifying ${
            TemperatureType[this.type]
    ...
    this.platform.log.debug(
      `${this.deviceName}: Set characteristic "${
        TemperatureType[this.type]
      } temperature" to: ${value}`
    );
    ...
    try {
      const response = await axios.put(
        this.platform.getActionsUrl(this.serialNumber),
        {
          targetTemperature: [{ zone: this.DEFAULT_ZONE, value: value }], // Setting is done in Celsius, retrieving returns centi-Celsius.
        },
        this.platform.getHttpRequestConfig()
      );

      this.platform.log.debug(
        `${this.deviceName}: Set target temperature response code: ${response.status}: "${response.statusText}"`
      );
    } catch (response) {
      if (response.response && response.response.status === 500) {
        this.platform.log.warn(
          `${this.deviceName}: Set target temperature: ignoring Miele API fault reply code ` +
...
export class MieleTemperatureUnitCharacteristic extends MieleBaseCharacteristic {
  constructor(platform: MieleAtHomePlatform, service: Service) {
    super(
      platform,
      service,
  ...
  async set(value: CharacteristicValue, callback: CharacteristicSetCallback) {
    ...
    try {
      ...
      if (response.data[action]) {
        ...
        this.platform.log.info(
          `${this.deviceName} (${this.serialNumber}): ` +
            `Ignoring request to power ${
              value ? "on" : "off"
            } the device: not allowed in current device state. ` +
